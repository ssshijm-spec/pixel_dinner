...
- ☐ HUD 패널·팝업 숫자·말풍선 글자가 확대해도 뭉개지지 않고 선명함(창을 늘려
  스케일을 키워도 마찬가지).
- ☐ 음식 대기(`WAIT_FOOD`) 말풍선이 그 손님이 실제로 주문한 요리의 색·모양으로
  표시되고, 아래에 요리명(한글)이 함께 보임(모든 손님이 같은 아이콘이 아님).
- ☐ 음식 말풍선이 다른 말풍선(?, $, ~, !)보다 눈에 띄게 크고, 어느 테이블에
  앉은 손님이어도 HUD 상단바나 화면 밖으로 잘리지 않음.
